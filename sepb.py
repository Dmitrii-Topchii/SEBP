...
class SparseMethodLinearIntermediate(nn.Module):
    """nn.Linear replacement for intermediate layers (Expands dimension)."""
...
class SparseMethodLinearOutput(nn.Module):
    """nn.Linear replacement for output layers (Reduces dimension)."""
    def __init__(self, in_features, out_features, layer_idx, bias=True, device=None, dtype=None):
        super().__init__(); factory_kwargs={'device': device, 'dtype': dtype}
        self.in_features=in_features; self.out_features=out_features; self.layer_idx = layer_idx
        self.weight = Parameter(torch.empty((out_features, in_features), **factory_kwargs))
...
def apply_sebp_to_model(model, sparse_rows=50, verbose=True):
    """
    Main entry point to inject SEBP into Llama, BERT, or RoBERTa.
    Detects architecture and replaces Linear layers with SparseMethodLinear layers.
    """
    SparseMethodLinearFunction._sparse_rows_to_keep = sparse_rows
    
    count = 0
    #regex to find layer index in standard HF naming (e.g. "layers.0", "h.5")
    layer_idx_pattern = re.compile(r"\.(layers|h|block)\.(\d+)\.")

    def replace_recursion(module, parent_name=""):
        nonlocal count
        for name, child in module.named_children():
            full_name = f"{parent_name}.{name}" if parent_name else name
            
            # Try to find which transformer block we are in
            match = layer_idx_pattern.search(full_name)
            layer_idx = int(match.group(2)) if match else 0
            
            if isinstance(child, nn.Linear):
                # Llama / Qwen: q, k, v, gate, up -> Intermediate
                # Llama / Qwen: o, down -> Output
                # BERT / RoBERTa: query, key, value, dense(in intermediate) -> Intermediate
                # BERT / RoBERTa: dense(in output) -> Output
                
                is_target = False
                is_output_type = False # False = Intermediate, True = Output

                # Llama / Mistral / Qwen Detection
                ...
                # BERT / RoBERTa Detection
                elif any(x in name for x in ['query', 'key', 'value']):
                    is_target = True
                    is_output_type = False
                elif 'dense' in name:
                    # Usually "intermediate.dense" (up) and "output.dense" (down).
                    if "output" in parent_name:
                        is_target = True
                        is_output_type = True
                    else:
                        is_target = True
                ...
                if is_target:
                    if is_output_type:
                        new_layer = SparseMethodLinearOutput(
                            child.in_features, child.out_features, 
                            layer_idx=layer_idx, bias=child.bias is not None,
                            device=child.weight.device, dtype=child.weight.dtype
                        )
                    else:
                        new_layer = SparseMethodLinearIntermediate(
                            child.in_features, child.out_features, 
                            layer_idx=layer_idx, bias=child.bias is not None,
                            device=child.weight.device, dtype=child.weight.dtype
                        )
                    
                    new_layer.from_linear(child)
                    setattr(module, name, new_layer)
                    count += 1
            else:
                replace_recursion(child, full_name)

    replace_recursion(model)
    if verbose:
        print(f"   SEBP Injection Complete.")
        print(f"   Replaced {count} layers.")
        print(f"   Configured Sparsity: Keep Top-{sparse_rows} rows.")
    return model